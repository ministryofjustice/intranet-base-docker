...
RUN add-apt-repository -y ppa:ondrej/php && \
    ...
    apt-get install -y \
        ...
        nginx nginx-extras\
        mariadb-client-${mdbv} \
        python3-pip libfuse-dev nullmailer git nano nodejs build-essential unzip && \
    apt-get clean && \
...
# Configure php-fpm
    mv /tmp/conf/php-fpm/php-fpm.conf /etc/php/${phpv}/fpm && \
    ...
    cat /tmp/conf/php-fpm/xdebug.ini >> /etc/php/${phpv}/mods-available/xdebug.ini && \
    phpdismod xdebug && \
...
# Configure Services
    ...
    mkdir /etc/service/nginx;      mv /etc/service/nginx.sh /etc/service/nginx/run;           chmod +x /etc/service/nginx/run && \
    mkdir /etc/service/nullmailer; mv /etc/service/nullmailer.sh /etc/service/nullmailer/run; chmod +x /etc/service/nullmailer/run && \
    mkdir /etc/service/php-fpm;    mv /etc/service/php-fpm.sh /etc/service/php-fpm/run;       chmod +x /etc/service/php-fpm/run && \
    mkdir /etc/service/yas3fs;     mv /etc/service/yas3fs.sh /etc/service/yas3fs/run;         chmod +x /etc/service/yas3fs/run && \
...
# Generate the Pingdom IP address allow-lists
    chmod +x /tmp/build/generate-pingdom-allow-list.sh && sleep 1 && \
